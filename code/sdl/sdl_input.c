...
static keyNum_t IN_KeyFor(SDL_GameControllerButton sdlButton) {
  ...
  case SDL_CONTROLLER_BUTTON_LEFTSTICK:
    return K_PAD0_LEFTSTICK_CLICK;
  case SDL_CONTROLLER_BUTTON_RIGHTSTICK:
    return K_PAD0_RIGHTSTICK_CLICK;
  case SDL_CONTROLLER_BUTTON_LEFTSHOULDER:
    return K_PAD0_LEFTSHOULDER;
  case SDL_CONTROLLER_BUTTON_RIGHTSHOULDER:
    return K_PAD0_RIGHTSHOULDER;
  case SDL_CONTROLLER_BUTTON_DPAD_UP:
    return K_PAD0_DPAD_UP;
  case SDL_CONTROLLER_BUTTON_DPAD_DOWN:
    return K_PAD0_DPAD_DOWN;
  case SDL_CONTROLLER_BUTTON_DPAD_LEFT:
    return K_PAD0_DPAD_LEFT;
  case SDL_CONTROLLER_BUTTON_DPAD_RIGHT:
    return K_PAD0_DPAD_RIGHT;
#if SDL_VERSION_ATLEAST(2, 0, 14)
  case SDL_CONTROLLER_BUTTON_MISC1:
    return K_PAD0_MISC1;
  case SDL_CONTROLLER_BUTTON_PADDLE1:
    return K_PAD0_PADDLE1;
  case SDL_CONTROLLER_BUTTON_PADDLE2:
    return K_PAD0_PADDLE2;
  case SDL_CONTROLLER_BUTTON_PADDLE3:
    return K_PAD0_PADDLE3;
  case SDL_CONTROLLER_BUTTON_PADDLE4:
    return K_PAD0_PADDLE4;
  case SDL_CONTROLLER_BUTTON_TOUCHPAD:
    return K_PAD0_TOUCHPAD;
#endif
...
static void IN_GamepadMove(void) {
  ...
  for (i = 0; i < SDL_CONTROLLER_BUTTON_MAX; i++) {
    ...
    if (pressed != stick_state.buttons[i]) {
      Com_QueueEvent(in_eventTime, SE_KEY, IN_KeyFor(sdlButton), pressed, 0,
                     NULL);
...
static void IN_ProcessEvents(void) {
  ...
  while (SDL_PollEvent(&e)) {
    ...
    case SDL_DROPFILE: {
      ...
      // Handle macOS open URL event. URL protocol scheme must be set in
      // Info.plist.
      if (!Q_strncmp(filename, PROTOCOL_HANDLER ":",
                     strlen(PROTOCOL_HANDLER ":"))) {
