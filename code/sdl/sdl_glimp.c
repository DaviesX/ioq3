...
static void APIENTRY GLimp_GLES_DepthRange(GLclampd near_val,
                                           GLclampd far_val) {
...
#define GLE(ret, name, ...)                                                    \
  ...
  if (fixedFunction) {
    if (QGL_VERSION_ATLEAST(1, 1)) {
      ...
      Com_Error(ERR_FATAL,
                "Unsupported OpenGL Version (%s), OpenGL 1.1 is required",
                version);
    }
  } else {
...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder,
  ...
  if (!fixedFunction) {
    ...
    preferOpenGLES = (r_preferOpenGLES->integer == 1 ||
                      (r_preferOpenGLES->integer == -1 &&
                       profileMask == SDL_GL_CONTEXT_PROFILE_ES));

    if (preferOpenGLES) {
#ifdef __EMSCRIPTEN__
      // WebGL 2.0 isn't fully backward compatible so you have to ask for it
      // specifically
  ...
  for (i = 0; i < 16; i++) {
    ...
    for (type = 0; type < numContexts; type++) {
      ...
      if (!SDL_glContext) {
        ...
        SDL_window = NULL;
        ri.Printf(PRINT_ALL,
                  "SDL_GL_CreateContext() for %s context failed: %s\n",
...
static void GLimp_InitExtensions(qboolean fixedFunction) {
  ...
  if ((QGLES_VERSION_ATLEAST(2, 0) ||
       SDL_GL_ExtensionSupported("GL_ARB_texture_compression")) &&
...
void GLimp_Init(qboolean fixedFunction) {
  ...
  r_preferOpenGLES =
      ri.Cvar_Get("r_preferOpenGLES", "-1", CVAR_ARCHIVE | CVAR_LATCH);
