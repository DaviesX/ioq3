...
void GLSL_InitGPUShaders(void) {
  ...
  // OpenGL ES may not have enough attributes to fit ones used for vertex
  // animation
  ...
  for (i = 0; i < LIGHTDEF_COUNT; i++) {
    ...
    qboolean fastLight =
        (qboolean)(!(r_normalMapping->integer || r_specularMapping->integer));
    ...
    if (lightType) {
      ...
      if (r_normalMapping->integer) {
        ...
        if ((i & LIGHTDEF_USE_PARALLAXMAP) &&
          ...
          Q_strcat(extradefines, 1024,
                   va("#define r_parallaxMapOffset %f\n",
  ...
  for (i = 0; i < SHADOWMAPDEF_COUNT; i++) {
    ...
    if ((i & SHADOWMAPDEF_USE_VERTEX_ANIMATION) &&
        !glRefConfig.gpuVertexAnimation)
      continue;
  ...
  // GLSL 1.10+ or GL_EXT_shadow_samplers extension are required for
  // sampler2DShadow type
  if (glRefConfig.glslMajorVersion > 1 ||
      (glRefConfig.glslMajorVersion == 1 &&
       glRefConfig.glslMinorVersion >= 10) ||
  ...
  if (!GLSL_InitGPUShader(&tr.greyscaleShader, "greyscale", attribs, qtrue,
                          extradefines, qtrue, fallbackShader_greyscale_vp,
                          fallbackShader_greyscale_fp)) {
  ...
  // GLSL 1.10+ or GL_OES_standard_derivatives extension are required for dFdx()
  // and dFdy() GLSL functions
  if (glRefConfig.glslMajorVersion > 1 ||
      (glRefConfig.glslMajorVersion == 1 &&
       glRefConfig.glslMinorVersion >= 10) ||
    ...
    if (!GLSL_InitGPUShader(&tr.ssaoShader, "ssao", attribs, qtrue,
                            extradefines, qtrue, fallbackShader_ssao_vp,
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (!GLSL_InitGPUShader(&tr.depthBlurShader[i], "depthBlur", attribs,
                              qtrue, extradefines, qtrue,
