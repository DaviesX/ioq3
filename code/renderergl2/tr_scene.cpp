...
void R_AddPolygonSurfaces(void) {
  ...
  for (i = 0, poly = tr.refdef.polys; i < tr.refdef.numPolys; i++, poly++) {
    ...
    R_AddDrawSurf((surfaceType_t *)poly, sh, poly->fogIndex & fogMask, qfalse,
                  qfalse, 0 /*cubeMap*/);
  }
}
...
void RE_AddPolyToScene(qhandle_t hShader, int numVerts, const polyVert_t *verts,
  ...
  if (!hShader) {
    ...
    // it's the default shader.
    // ri.Printf( PRINT_WARNING, "WARNING: RE_AddPolyToScene: NULL poly
    // shader\n"); return;
  }
...
void RE_AddRefEntityToScene(const refEntity_t *ent) {
  ...
  backEndData->entities[r_numentities].mirrored =
      (qboolean)((DotProduct(ent->axis[2], cross) < 0.f));
