...
static void DrawNormals(shaderCommands_t *input) {
  ...
  // Skip animated entities that are interpolating; otherwise normals will
  // appear to "swim".
  ...
  if (boundVao) {
    ...
    usePacked = (qboolean)((a->type == GL_SHORT && a->normalized));
  } else {
    ...
    usePacked = (qboolean)(fabsf(lp - 1.0f) <= fabsf(lf - 1.0f));
  }
  ...
  qglBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * numVerts, lineVertices,
                GL_STREAM_DRAW);

  shaderProgram_t *program = &tr.textureColorShader;
  GLSL_BindProgram(program);
  GLSL_SetUniformMat4(program, UNIFORM_MODELVIEWPROJECTIONMATRIX,
                      glState.modelviewProjection);
  ...
  qglVertexAttribPointer(ATTR_INDEX_POSITION, 3, GL_FLOAT, GL_FALSE, 0,
                         (const void *)0);
...
static void ComputeTexMods(shaderStage_t *pStage, int bundleNum,
  ...
  for (tm = 0; tm < bundle->numTexMods; tm++) {
    ...
    default:
      ...
      tmpmatrix[4] = matrix[0] * currentmatrix[4] +
                     matrix[2] * currentmatrix[5] + matrix[4];
      tmpmatrix[5] = matrix[1] * currentmatrix[4] +
...
static void ComputeShaderColors(shaderStage_t *pStage, vec4_t baseColor,
  ...
  qboolean isBlend =
      (qboolean)(((blend & GLS_SRCBLEND_BITS) == GLS_SRCBLEND_DST_COLOR) ||
  ...
  qboolean is2DDraw = (qboolean)(backEnd.currentEntity == &backEnd.entity2D);
...
static void RB_IterateStagesGeneric(shaderCommands_t *input) {
  ...
  qboolean renderToCubemap =
      (qboolean)(tr.renderCubeFbo && glState.currentFBO == tr.renderCubeFbo);
  ...
  for (stage = 0; stage < MAX_SHADER_STAGES; stage++) {
    ...
    if (backEnd.depthFill) {
      ...
      if ((r_lightmap->integer == 1 || r_lightmap->integer == 2) &&
        ...
        qboolean light = (qboolean)((pStage->glslShaderIndex &
                                     LIGHTDEF_LIGHTTYPE_MASK) != 0);
        qboolean fastLight = (qboolean)(!(r_normalMapping->integer ||
...
void RB_StageIteratorGeneric(void) {
  ...
  if (input->shader->cullType == CT_TWO_SIDED) {
    ...
    qboolean cullFront =
        (qboolean)((input->shader->cullType == CT_FRONT_SIDED));
