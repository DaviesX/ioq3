    ...
    for (i = 0; i < 6; i++) {
      ...
      shader.sky.outerbox[i] =
          R_FindImageFile((char *)pathname, IMGTYPE_COLORALPHA,
                          (imgFlags_t)(imgFlags | IMGFLAG_CLAMPTOEDGE));
  ...
  if (r_normalMapping->integer) {
    ...
    if (normal) {
      ...
      imgFlags_t normalFlags =
          (imgFlags_t)((diffuseImg->flags & ~IMGFLAG_GENNORMALMAP) |
    ...
    while (1) {
      ...
      case RC_DRAW_SURFS: {
        ...
        for (i = 0, drawSurf = ds_cmd->drawSurfs; i < ds_cmd->numDrawSurfs;
             i++, drawSurf++) {
          R_DecomposeSort(drawSurf->sort, &entityNum, &pShader, &fogNum,
...
static void FixFatLightmapTexCoords(void) {
  ...
  for (stage = 0; stage < MAX_SHADER_STAGES; stage++) {
    ...
    if (pStage->bundle[0].isLightmap) {
      // fix tcMod transform for internal lightmaps, it may be used by q3map2
      // lightstyles
      if (pStage->bundle[0].tcGen == TCGEN_LIGHTMAP) {
      ...
      // fix tcGen environment for internal lightmaps to be limited to the
      // sub-image of the atlas this is done last so other tcMods are applied
      // first in the 0.0 to 1.0 space
      if (pStage->bundle[0].tcGen == TCGEN_ENVIRONMENT_MAPPED) {
        if (pStage->bundle[0].numTexMods == TR_MAX_TEXMODS) {
          ri.Printf(PRINT_DEVELOPER,
                    "WARNING: too many tcmods to fix lightmap texcoords for "
                    "r_mergeLightmaps in shader '%s'",
                    shader.name);
        } else {
          ...
          tmi->translate[0] =
              (lightmapnum % tr.fatLightmapCols) / (float)tr.fatLightmapCols;
          tmi->translate[1] =
              (lightmapnum / tr.fatLightmapCols) / (float)tr.fatLightmapRows;

          tmi->type = TMOD_TRANSFORM;
        }
      }
    }
    // add a tcMod transform for external lightmaps to convert back to the
    // original texcoords
      ...
      if (pStage->bundle[0].numTexMods == TR_MAX_TEXMODS) {
        ...
        if (size) {
          memmove(&pStage->bundle[0].texMods[1], &pStage->bundle[0].texMods[0],
                  size);
        }
...
===============
*/
static void InitShaderEx(const char *name, int lightmapIndex,
...
shader_t *R_FindShaderEx(const char *name, int lightmapIndex,
                         qboolean mipRawImage, int realLightmapIndex) {
  char strippedName[MAX_QPATH];
  int hash;
  char *shaderText;
  image_t *image;
