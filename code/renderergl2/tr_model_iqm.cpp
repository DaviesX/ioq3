...
static qboolean IQM_CheckRange(iqmHeader_t *header, int offset, int count,
  ...
  return (qboolean)((count <= 0 || offset <= 0 || offset > header->filesize ||
                     offset + count * size < 0 ||
...
static void QuatSlerp(const quat_t from, const quat_t _to, float fraction,
                      quat_t out) {
  ...
  // cos() of angle
  cosAngle =
      from[0] * _to[0] + from[1] * _to[1] + from[2] * _to[2] + from[3] * _to[3];

  // negative handling is needed for taking shortest path (required for model
  // joints)
  if (cosAngle < 0.0f) {
    cosAngle = -cosAngle;
    to[0] = -_to[0];
...
qboolean R_LoadIQM(model_t *mod, void *buffer, int filesize,
  ...
  if (header->num_joints) {
    ...
    size +=
        header->num_joints * 12 * sizeof(float); // inverse bind joint matricies
  }
  if (header->num_poses) {
    size += header->num_poses * header->num_frames *
            sizeof(iqmTransform_t); // pose transforms
  }
  if (header->ofs_bounds) {
  ...
  if (header->num_poses) {
    ...
    dataPtr += header->num_poses * header->num_frames *
               sizeof(iqmTransform_t); // pose transforms
  }
  if (header->ofs_bounds) {
  ...
  if (header->num_joints) {
    ...
    for (i = 0; i < header->num_joints; i++, joint++) {
      ...
      if (joint->parent >= 0) {
        Matrix34Multiply(iqmData->bindJoints + 12 * joint->parent, baseFrame,
                         mat);
        mat += 12;
        Matrix34Multiply(invBaseFrame,
                         iqmData->invBindJoints + 12 * joint->parent, matInv);
        matInv += 12;
      } else {
  ...
  if (iqmData->num_surfaces &&
    ...
    iqmData->vaoSurfaces = (struct srfVaoIQModel_s *)ri.Hunk_Alloc(
...
void R_AddIQMSurfaces(trRefEntity_t *ent) {
  ...
  personalModel =
      (qboolean)((ent->e.renderfx & RF_THIRD_PERSON) &&
...
static void ComputePoseMats(iqmData_t *data, int frame, int oldframe,
  ...
  if (oldframe == frame) {
    ...
    for (i = 0; i < data->num_poses; i++, oldpose++, pose++, relativeJoint++) {
      relativeJoint->translate[0] =
          oldpose->translate[0] * backlerp + pose->translate[0] * lerp;
      relativeJoint->translate[1] =
          oldpose->translate[1] * backlerp + pose->translate[1] * lerp;
      relativeJoint->translate[2] =
          oldpose->translate[2] * backlerp + pose->translate[2] * lerp;

      relativeJoint->scale[0] =
          oldpose->scale[0] * backlerp + pose->scale[0] * lerp;
      relativeJoint->scale[1] =
          oldpose->scale[1] * backlerp + pose->scale[1] * lerp;
      relativeJoint->scale[2] =
          oldpose->scale[2] * backlerp + pose->scale[2] * lerp;

      QuatSlerp(oldpose->rotate, pose->rotate, lerp, relativeJoint->rotate);
    }
  }

  // multiply by inverse of bind pose and parent 'pose mat' (bind pose transform
  // matrix)
  relativeJoint = relativeJoints;
  ...
  if (data->num_poses > 0) {
    ...
    for (i = 0; i < surf->num_influences; i++) {
      ...
      if (data->blendWeightsType == IQM_FLOAT) {
        ...
        blendWeights[0] =
            (float)data->influenceBlendWeights.b[4 * influence + 0] / 255.0f;
        blendWeights[1] =
            (float)data->influenceBlendWeights.b[4 * influence + 1] / 255.0f;
        blendWeights[2] =
            (float)data->influenceBlendWeights.b[4 * influence + 2] / 255.0f;
        blendWeights[3] =
            (float)data->influenceBlendWeights.b[4 * influence + 3] / 255.0f;
      }
