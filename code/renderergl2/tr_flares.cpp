...
void RB_TestFlare(flare_t *f) {
  ...
  R_TransformModelToClip(f->origin, backEnd.ori.modelMatrix,
                         backEnd.viewParms.projectionMatrix, eyePos, clipPos);
  if (clipPos[3] <= 0.0f) {
    visible = qfalse; // behind eye
  } else {
    // Treat near-equal depths as visible, tiny margin to avoid false occlusion
    // from rounding
    ...
    visible =
        (qboolean)((flareDepth - depthBias) <= depth); // in front (with margin)
  }
...
void RB_RenderFlares(void) {
  ...
  if (r_flares->modified) {
    if (qglesMajorVersion >= 1 && !glRefConfig.readDepth) {
      ri.Printf(PRINT_WARNING, "OpenGL ES needs GL_NV_read_depth to read depth "
                               "to determine if flares are visible\n");
