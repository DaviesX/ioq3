...
void RB_RenderDrawSurfList(drawSurf_t *drawSurfs, int numDrawSurfs) {
  ...
  for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
    if (drawSurf->sort == (unsigned)oldSort &&
      ...
      if (backEnd.depthFill && shader &&
          (shader->sort != SS_OPAQUE && shader->sort != SS_PORTAL))
        continue;
...
void RE_UploadCinematic(int w, int h, int cols, int rows, const byte *data,
  ...
  if (cols != tr.scratchImage[client]->width ||
    ...
    if (qglesMajorVersion >= 1) {
      buffer = (byte *)ri.Hunk_AllocateTempMemory(3 * cols * rows);

      R_ConvertTextureFormat(data, cols, rows, GL_RGB, GL_UNSIGNED_BYTE,
                             buffer);
      qglTextureImage2DEXT(texture, GL_TEXTURE_2D, 0, GL_RGB, cols, rows, 0,
...
const void *RB_DrawSurfs(const void *data) {
  ...
  isShadowView = (qboolean)(!!(backEnd.viewParms.flags & VPF_DEPTHSHADOW));
  ...
  if (!isShadowView) {
    ...
    if (glRefConfig.framebufferObject && r_drawSunRays->integer) {
      ...
      if (glRefConfig.occlusionQuery) {
        ...
        qglBeginQuery(glRefConfig.occlusionQueryTarget,
  ...
  backEnd.viewParms.flags = (viewParmFlags_t)(0);
...
static void RB_DrawGreyscale(const FBO_t *src) {
  ...
  GLSL_SetUniformFloat(&tr.greyscaleShader, UNIFORM_GREYSCALE,
                       backEnd.greyscale);
...
static void RB_PresentToScreen(void) {
  ...
  if (tr.renderFbo && tr.renderFbo->colorImage[0]) {
    ...
    // Resolving an RGB16F MSAA FBO to the screen messes with the brightness, so
    // resolve to an RGB16F FBO first
    FBO_FastBlit(tr.renderFbo, NULL, tr.msaaResolveFbo, NULL,
...
const void *RB_PostProcess(const void *data) {
  ...
  if (srcFbo) {
    if (r_hdr->integer && (r_toneMap->integer || r_forceToneMap->integer)) {
      autoExposure =
          (qboolean)(r_autoExposure->integer || r_forceAutoExposure->integer);
      ...
      FBO_FastBlit(tr.screenScratchFbo, srcBox, srcFbo, srcBox,
                   GL_COLOR_BUFFER_BIT, GL_NEAREST);
      ...
      FBO_BlitFromTexture(tr.whiteImage, NULL, NULL, srcFbo, srcBox, NULL,
  ...
  if (0 && r_sunlightMode->integer) {
    ...
    FBO_BlitFromTexture(tr.sunShadowDepthImage[0], NULL, NULL, dstFbo, dstBox2,
                        NULL, NULL, 0);
    VectorSet4(dstBox2, 128, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.sunShadowDepthImage[1], NULL, NULL, dstFbo, dstBox2,
                        NULL, NULL, 0);
    VectorSet4(dstBox2, 256, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.sunShadowDepthImage[2], NULL, NULL, dstFbo, dstBox2,
                        NULL, NULL, 0);
    VectorSet4(dstBox2, 384, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.sunShadowDepthImage[3], NULL, NULL, dstFbo, dstBox2,
                        NULL, NULL, 0);
  }
  ...
  if (0 && r_shadows->integer == 4) {
    ...
    FBO_BlitFromTexture(tr.pshadowMaps[0], NULL, NULL, dstFbo, dstBox2, NULL,
                        NULL, 0);
    VectorSet4(dstBox2, 512 + 128, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.pshadowMaps[1], NULL, NULL, dstFbo, dstBox2, NULL,
                        NULL, 0);
    VectorSet4(dstBox2, 512 + 256, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.pshadowMaps[2], NULL, NULL, dstFbo, dstBox2, NULL,
                        NULL, 0);
    VectorSet4(dstBox2, 512 + 384, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.pshadowMaps[3], NULL, NULL, dstFbo, dstBox2, NULL,
                        NULL, 0);
  }
  ...
  if (0) {
    ...
    FBO_BlitFromTexture(tr.renderDepthImage, NULL, NULL, dstFbo, dstBox2, NULL,
                        NULL, 0);
    VectorSet4(dstBox2, 512, glConfig.vidHeight - 256, 256, 256);
    FBO_BlitFromTexture(tr.screenShadowImage, NULL, NULL, dstFbo, dstBox2, NULL,
                        NULL, 0);
  }
  ...
  if (0) {
    ivec4_t dstBox2;
    VectorSet4(dstBox2, 256, glConfig.vidHeight - 256, 256, 256);
    FBO_BlitFromTexture(tr.sunRaysImage, NULL, NULL, dstFbo, dstBox2, NULL,
                        NULL, 0);
  }
