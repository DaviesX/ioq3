...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer, int bufferSize,
  ...
  for (i = 0; i < md3Model->numSurfaces; i++) {
    ...
    surf->shaderIndexes = shaderIndex =
        (int *)ri.Hunk_Alloc(sizeof(*shaderIndex) * md3Surf->numShaders, h_low);
    ...
    surf->indexes = tri = (glIndex_t *)ri.Hunk_Alloc(
        sizeof(*tri) * 3 * md3Surf->numTriangles, h_low);
    ...
    surf->verts = v = (mdvVertex_t *)ri.Hunk_Alloc(
    ...
    // calc tangent spaces
    {
      vec3_t *sdirs = (vec3_t *)ri.Malloc(sizeof(*sdirs) * surf->numVerts *
                                          mdvModel->numFrames);
      vec3_t *tdirs = (vec3_t *)ri.Malloc(sizeof(*tdirs) * surf->numVerts *
                                          mdvModel->numFrames);
  ...
  if (mdvModel->numFrames > 1 && !glRefConfig.gpuVertexAnimation) {
    ...
    mdvModel->vaoSurfaces = (srfVaoMdvMesh_t *)ri.Hunk_Alloc(
