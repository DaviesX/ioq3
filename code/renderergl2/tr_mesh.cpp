...
void R_AddMD3Surfaces(trRefEntity_t *ent) {
  ...
  for (i = 0; i < model->numSurfaces; i++) {
    ...
    if (!personalModel && r_shadows->integer == 2 && fogNum == 0 &&
      ...
      R_AddDrawSurf((surfaceType_t *)drawSurf, tr.shadowShader, 0, qfalse,
                    qfalse, 0);
    }
    ...
    if (r_shadows->integer == 3 && fogNum == 0 &&
      ...
      R_AddDrawSurf((surfaceType_t *)drawSurf, tr.projectionShadowShader, 0,
                    qfalse, qfalse, 0);
    }

    // don't add third_person objects if not viewing through a portal
    if (!personalModel) {
      R_AddDrawSurf((surfaceType_t *)drawSurf, shader, fogNum, qfalse, qfalse,
                    cubemapIndex);
    }
