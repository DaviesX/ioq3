...
void R_SetupFrustum(viewParms_t *dest, float xmin, float xmax, float ymax,
  ...
  if (stereoSep == 0 && xmin == -xmax) {
    ...
    VectorMA(dest->frustum[0].normal, adjleg, dest->ori.axis[1],
             dest->frustum[0].normal);

    VectorScale(dest->ori.axis[0], oppleg, dest->frustum[1].normal);
    VectorMA(dest->frustum[1].normal, -adjleg, dest->ori.axis[1],
             dest->frustum[1].normal);
  } else {
    // In stereo rendering, due to the modification of the projection matrix,
    // dest->ori.origin is not the actual origin that we're rendering so offset
    ...
    VectorMA(dest->frustum[0].normal, zProj / length, dest->ori.axis[1],
             dest->frustum[0].normal);

    oppleg = xmin + stereoSep;
    length = sqrt(oppleg * oppleg + zProj * zProj);
    VectorScale(dest->ori.axis[0], -oppleg / length, dest->frustum[1].normal);
    VectorMA(dest->frustum[1].normal, -zProj / length, dest->ori.axis[1],
             dest->frustum[1].normal);
  }
  ...
  VectorMA(dest->frustum[2].normal, adjleg, dest->ori.axis[2],
           dest->frustum[2].normal);

  VectorScale(dest->ori.axis[0], oppleg, dest->frustum[3].normal);
  VectorMA(dest->frustum[3].normal, -adjleg, dest->ori.axis[2],
...
qboolean R_GetPortalOrientations(drawSurf_t *drawSurf, int entityNum,
  ...
  if (entityNum != REFENTITYNUM_WORLD) {
    ...
    originalPlane.dist =
        originalPlane.dist + DotProduct(originalPlane.normal, tr.ori.origin);
  } else {
    plane = originalPlane;
  }
...
static qboolean SurfIsOffscreen(const drawSurf_t *drawSurf,
  ...
  for (i = 0; i < tess.numVertexes; i++) {
    ...
    R_TransformModelToClip(tess.xyz[i], tr.ori.modelMatrix,
...
===============
*/
static ID_INLINE void R_Radix(int offset, int size, drawSurf_t *source,
                              drawSurf_t *dest) {
  int count[256] = {0};
  int index[256];
  int i;
...
void R_RenderDlightCubemaps(const refdef_t *fd) {
  ...
  for (i = 0; i < tr.refdef.num_dlights; i++) {
    ...
    shadowParms.flags =
        (viewParmFlags_t)(VPF_SHADOWMAP | VPF_DEPTHSHADOW | VPF_NOVIEWMODEL);
  ...
  for (i = 0; i < tr.refdef.num_pshadows; i++) {
...

    {
...

      {
        ...
        VectorScale(dest->ori.axis[1], 1.0f, dest->frustum[0].normal);
        VectorMA(dest->ori.origin, -shadow->viewRadius, dest->frustum[0].normal,
                 pop);
        dest->frustum[0].dist = DotProduct(pop, dest->frustum[0].normal);

        VectorScale(dest->ori.axis[1], -1.0f, dest->frustum[1].normal);
        VectorMA(dest->ori.origin, -shadow->viewRadius, dest->frustum[1].normal,
                 pop);
        dest->frustum[1].dist = DotProduct(pop, dest->frustum[1].normal);

        VectorScale(dest->ori.axis[2], 1.0f, dest->frustum[2].normal);
        VectorMA(dest->ori.origin, -shadow->viewRadius, dest->frustum[2].normal,
                 pop);
        dest->frustum[2].dist = DotProduct(pop, dest->frustum[2].normal);

        VectorScale(dest->ori.axis[2], -1.0f, dest->frustum[3].normal);
        VectorMA(dest->ori.origin, -shadow->viewRadius, dest->frustum[3].normal,
                 pop);
        dest->frustum[3].dist = DotProduct(pop, dest->frustum[3].normal);

        VectorScale(dest->ori.axis[0], -1.0f, dest->frustum[4].normal);
        VectorMA(dest->ori.origin, -shadow->lightRadius,
...
void R_RenderSunShadowMaps(const refdef_t *fd, int level) {
...

  {
    ...
    shadowParms.flags = (viewParmFlags_t)(VPF_DEPTHSHADOW | VPF_DEPTHCLAMP |
                                          VPF_ORTHOGRAPHIC | VPF_NOVIEWMODEL);
