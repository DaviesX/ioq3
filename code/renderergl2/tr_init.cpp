...
byte *RB_ReadPixels(int x, int y, int width, int height, size_t *offset,
  ...
  buffer = (byte *)ri.Hunk_AllocateTempMemory(padwidth * height + *offset +
                                              packAlign - 1);
  ...
  if (format == GL_RGBA) {
    for (yin = 0; yin < height; yin++) {
      for (xin = 0, xout = 0; xout < linelen; xin += 4, xout += 3) {
        bufstart[yin * padwidth + xout + 0] =
            bufstart[yin * padwidth + xin + 0];
        bufstart[yin * padwidth + xout + 1] =
            bufstart[yin * padwidth + xin + 1];
        bufstart[yin * padwidth + xout + 2] =
            bufstart[yin * padwidth + xin + 2];
      }
    }
  }
...
void GfxInfo_f(void) {
  ...
  // glConfig.extensions_string is a limited length so get the full list
  // directly
  if (qglGetStringi) {
    GLint numExtensions;
    int i;
...
void R_Register(void) {
  ...
  r_parallaxMapOffset =
      ri.Cvar_Get("r_parallaxMapOffset", "0", CVAR_ARCHIVE | CVAR_LATCH);
  ...
  // Under Emscripten we don't throttle framerate with com_maxfps by default, so
  // enable vsync by default instead.
  r_swapInterval =
      ri.Cvar_Get("r_swapInterval", "1", CVAR_ARCHIVE | CVAR_LATCH);
#else
  r_swapInterval =
...
void R_Init(void) {
  ...
  ptr = (byte *)ri.Hunk_Alloc(sizeof(*backEndData) +
