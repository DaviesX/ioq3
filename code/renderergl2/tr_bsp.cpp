...
static void R_LoadLightmaps(lump_t *l, lump_t *surfs) {
  imgFlags_t imgFlags =
      (imgFlags_t)(IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION |
                   IMGFLAG_CLAMPTOEDGE);
  byte *buf, *buf_p;
  dsurface_t *surf;
  int len;
  ...
  tr.lightmaps =
      (image_t **)ri.Hunk_Alloc(tr.numLightmaps * sizeof(image_t *), h_low);
      ...
      for (j = 0; j < tr.lightmapSize * tr.lightmapSize; j++) {
        if (hdrLightmap) {
          ...
          if (r_lightmap->integer ==
              2) { // color code by intensity as development tool	(FIXME:
                   // check range)
            float r = buf_p[j * 3 + 0];
            float g = buf_p[j * 3 + 1];
            float b = buf_p[j * 3 + 2];
...
static shader_t *ShaderForShaderNum(int shaderNum, int lightmapNum) {
  ...
  shader =
      R_FindShaderEx(dsh->shader, FatLightmap(lightmapNum), qtrue, lightmapNum);
...
static void ParseFace(dsurface_t *ds, drawVert_t *verts, float *hdrVertColors,
  ...
  cv = (srfBspSurface_t *)(void *)surf->data;
  cv->surfaceType = SF_FACE;

  cv->numIndexes = numIndexes;
  cv->indexes =
      (glIndex_t *)ri.Hunk_Alloc(numIndexes * sizeof(cv->indexes[0]), h_low);
...
static void ParseFlare(dsurface_t *ds, drawVert_t *verts, msurface_t *surf,
  ...
  flare = (srfFlare_t *)(void *)surf->data;
...
void R_MovePatchSurfacesToHunk(void) {
  ...
  for (i = 0; i < s_worldData.numsurfaces; i++) {
    ...
    grid->indexes =
        (glIndex_t *)ri.Hunk_Alloc(grid->numIndexes * sizeof(glIndex_t), h_low);
...
static void R_LoadSurfaces(lump_t *surfs, lump_t *verts, lump_t *indexLump) {
  ...
  s_worldData.surfacesViewCount = (int *)ri.Hunk_Alloc(
      count * sizeof(*s_worldData.surfacesViewCount), h_low);
  s_worldData.surfacesDlightBits = (int *)ri.Hunk_Alloc(
      count * sizeof(*s_worldData.surfacesDlightBits), h_low);
  s_worldData.surfacesPshadowBits = (int *)ri.Hunk_Alloc(
  ...
  for (i = 0; i < count; i++, in++, out++) {
    ...
    case MST_PATCH:
      out->data =
          (surfaceType_t *)ri.Hunk_Alloc(sizeof(srfBspSurface_t), h_low);
      break;
...
void R_LoadLightGrid(lump_t *l) {
  ...
  if (r_hdr->integer) {
    ...
    if (hdrLightGrid) {
      ...
      w->lightGrid16 = (uint16_t *)ri.Hunk_Alloc(
          sizeof(w->lightGrid16) * 6 * numGridPoints, h_low);
...
void R_LoadEnvironmentJson(const char *baseName) {
  ...
  tr.cubemaps =
      (cubemap_t *)ri.Hunk_Alloc(tr.numCubemaps * sizeof(*tr.cubemaps), h_low);
...
void R_LoadCubemaps(void) {
  int i;
  imgFlags_t flags = (imgFlags_t)(IMGFLAG_CLAMPTOEDGE | IMGFLAG_MIPMAP |
...
void RE_LoadWorldMap(const char *name) {
  ...
  if (0) {
    ...
    lightGridSize =
        w->lightGridBounds[0] * w->lightGridBounds[1] * w->lightGridBounds[2];
    primaryLightGrid =
        (uint8_t *)ri.Malloc(lightGridSize * sizeof(*primaryLightGrid));
