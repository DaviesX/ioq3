...
void GLimp_InitExtraExtensions(void) {
  ...
  q_gl_version_at_least_3_0 = (qboolean)(QGL_VERSION_ATLEAST(3, 0));
  q_gl_version_at_least_3_2 = (qboolean)(QGL_VERSION_ATLEAST(3, 2));
...
#undef GLE
#define GLE(ret, name, ...)                                                    \
  qgl##name = (name##proc *)SDL_GL_GetProcAddress("gl" #name "EXT");
...
#define GLE(ret, name, ...)                                                    \
  ...
  if (q_gl_version_at_least_3_0 || SDL_GL_ExtensionSupported(extension)) {
    glRefConfig.framebufferObject =
        (qboolean)(!!r_ext_framebuffer_object->integer);
  ...
  if (q_gl_version_at_least_3_0 || SDL_GL_ExtensionSupported(extension)) {
    if (q_gl_version_at_least_3_0) {
      ...
      glRefConfig.vertexArrayObject =
          (qboolean)(!!r_arb_vertex_array_object->integer);
    }
  ...
  if (q_gl_version_at_least_3_0 || SDL_GL_ExtensionSupported(extension)) {
    glRefConfig.textureFloat = (qboolean)(!!r_ext_texture_float->integer);
  ...
  if (q_gl_version_at_least_3_2 || SDL_GL_ExtensionSupported(extension)) {
    glRefConfig.seamlessCubeMap =
        (qboolean)(!!r_arb_seamless_cube_map->integer);
  ...
  if (SDL_GL_ExtensionSupported(extension)) {
    qboolean useRgtc = (qboolean)(r_ext_compressed_textures->integer >= 1);

    if (useRgtc)
      glRefConfig.textureCompression =
          (textureCompressionRef_t)(glRefConfig.textureCompression | TCR_RGTC);
  ...
  glRefConfig.swizzleNormalmap =
      (qboolean)(r_ext_compressed_textures->integer &&
                 !(glRefConfig.textureCompression & TCR_RGTC));

  // GL_ARB_texture_compression_bptc
  extension = "GL_ARB_texture_compression_bptc";
  if (SDL_GL_ExtensionSupported(extension)) {
    qboolean useBptc = (qboolean)(r_ext_compressed_textures->integer >= 2);

    if (useBptc)
      glRefConfig.textureCompression =
          (textureCompressionRef_t)(glRefConfig.textureCompression | TCR_BPTC);
  ...
  if (SDL_GL_ExtensionSupported(extension)) {
    glRefConfig.directStateAccess =
        (qboolean)(!!r_ext_direct_state_access->integer);
  ...
  if (1) {
    ...
    sscanf(version_p, "%d.%d", &glRefConfig.glslMajorVersion,
