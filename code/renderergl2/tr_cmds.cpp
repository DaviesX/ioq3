...
void RE_BeginFrame(stereoFrame_t stereoFrame) {
  ...
  if (r_measureOverdraw->integer) {
    if (qglesMajorVersion >= 1 && !glRefConfig.readStencil) {
      ri.Printf(PRINT_WARNING, "OpenGL ES needs GL_NV_read_stencil to read "
                               "stencil bits to measure overdraw\n");
  ...
  if (glConfig.stereoEnabled) {
    ...
    if (qglesMajorVersion >= 1 && r_anaglyphMode->integer) {
      ri.Printf(PRINT_WARNING, "OpenGL ES does not support drawing to separate "
                               "buffer for anaglyph mode\n");
    ...
    if (r_anaglyphMode->integer) {
      ...
      if (stereoFrame == STEREO_LEFT) {
        ...
        if (!(colcmd =
                  (colorMaskCommand_t *)R_GetCommandBuffer(sizeof(*colcmd))))
          return;
      } else if (stereoFrame == STEREO_RIGHT) {
        clearDepthCommand_t *cldcmd;

        if (!(cldcmd =
                  (clearDepthCommand_t *)R_GetCommandBuffer(sizeof(*cldcmd))))
          return;
    ...
    if (cmd) {
      ...
      if (r_anaglyphMode->modified) {
        qglColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
        backEnd.colorMask[0] = (qboolean)(0);
        backEnd.colorMask[1] = (qboolean)(0);
        backEnd.colorMask[2] = (qboolean)(0);
        backEnd.colorMask[3] = (qboolean)(0);
        r_anaglyphMode->modified = qfalse;
      }
