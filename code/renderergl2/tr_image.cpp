...
==================
*/
#define ROW_PADDING(width, bpp, alignment)                                     \
  PAD((width) * (bpp), (alignment)) - (width) * (bpp)
void R_ConvertTextureFormat(const byte *in, int width, int height,
                            GLenum format, GLenum type, byte *out) {
  int x, y, rowPadding;
  ...
  if (format == GL_RGB && type == GL_UNSIGNED_BYTE) {
    ...
    ri.Error(ERR_DROP,
             "Unable to convert RGBA image to OpenGL format 0x%X and type 0x%X",
             format, type);
  }
}
...
static qboolean RawImage_ScaleToPower2(byte **data, int *inout_width,
  ...
  if (picmip && data && resampledBuffer && r_imageUpsample->integer &&
    ...
    *resampledBuffer =
        (byte *)ri.Hunk_AllocateTempMemory(finalwidth * finalheight * 4);
    ...
    while (scaled_width < finalwidth || scaled_height < finalheight) {
      ...
      FCBIByBlock(
          *resampledBuffer, scaled_width, scaled_height, clampToEdge,
          (qboolean)(type == IMGTYPE_NORMAL || type == IMGTYPE_NORMALHEIGHT));
    }
    ...
    *data = *resampledBuffer;
    ...
    if (data && resampledBuffer) {
      *resampledBuffer =
          (byte *)ri.Hunk_AllocateTempMemory(scaled_width * scaled_height * 4);
      ResampleTexture(*data, width, height, *resampledBuffer, scaled_width,
                      scaled_height);
      *data = *resampledBuffer;
    }
  ...
  scaled = (qboolean)((width != scaled_width) || (height != scaled_height));
...
static void RawImage_UploadTexture(GLuint texture, byte *data, int x, int y,
                                   ...
                                   GLenum picFormat, GLenum dataFormat,
                                   GLenum dataType, int numMips,
                                   GLenum internalFormat, imgType_t type,
                                   imgFlags_t flags, qboolean subtexture) {
  qboolean rgtc = (qboolean)(internalFormat == GL_COMPRESSED_RG_RGTC2);
  qboolean rgba8 =
      (qboolean)(picFormat == GL_RGBA8 || picFormat == GL_SRGB8_ALPHA8_EXT);
  qboolean rgba = (qboolean)(rgba8 || picFormat == GL_RGBA16);
  qboolean mipmap = (qboolean)(!!(flags & IMGFLAG_MIPMAP));
  int size, miplevel;
  qboolean lastMip = qfalse;
  byte *formatBuffer = NULL;

  if (qglesMajorVersion && rgba8 &&
      (dataFormat != GL_RGBA || dataType != GL_UNSIGNED_BYTE)) {
    formatBuffer = (byte *)ri.Hunk_AllocateTempMemory(4 * width * height);
  }

  miplevel = 0;
  do {
    lastMip = (qboolean)((width == 1 && height == 1) || !mipmap);
    ...
    if (!rgba) {
      ...
      if (rgba8 && rgtc)
        ...
        R_ConvertTextureFormat(data, width, height, dataFormat, dataType,
                               formatBuffer);
        qglTextureSubImage2DEXT(texture, target, miplevel, x, y, width, height,
                                dataFormat, dataType, formatBuffer);
      } else
        qglTextureSubImage2DEXT(texture, target, miplevel, x, y, width, height,
...
static void Upload32(byte *data, int x, int y, int width, int height,
                     GLenum picFormat, GLenum dataFormat, GLenum dataType,
                     int numMips, image_t *image, qboolean scaled) {
  int i, c;
  ...
  qboolean rgba8 =
      (qboolean)(picFormat == GL_RGBA8 || picFormat == GL_SRGB8_ALPHA8_EXT);
  qboolean mipmap =
      (qboolean)(!!(flags & IMGFLAG_MIPMAP) && (rgba8 || numMips > 1));
  qboolean cubemap = (qboolean)(!!(flags & IMGFLAG_CUBEMAP));
  ...
  if (cubemap) {
    for (i = 0; i < 6; i++) {
      ...
      RawImage_UploadTexture(image->texnum, data, x, y, width, height,
                             ...
                             dataFormat, dataType, numMips, internalFormat,
                             type, flags, qfalse);
      for (c = numMips; c; c--) {
    ...
    RawImage_UploadTexture(image->texnum, data, x, y, width, height,
                           GL_TEXTURE_2D, picFormat, dataFormat, dataType,
...
image_t *R_CreateImage2(const char *name, byte *pic, int width, int height,
  ...
  image = tr.images[tr.numImages] =
      (image_t *)ri.Hunk_Alloc(sizeof(image_t), h_low);
  ...
  // Convert image data format for OpenGL ES, data is converted for each mip
  // level
  if (qglesMajorVersion) {
    ...
    default:
      ri.Error(ERR_DROP,
               "Missing OpenGL ES support for image '%s' with internal format "
               "0x%X\n",
               name, internalFormat);
    }
  }
  ...
  miplevel = 0;
  do {
    lastMip = (qboolean)(!mipmap || (mipWidth == 1 && mipHeight == 1));
    if (cubemap) {
      int i;
  ...
  // Upload data.
  if (pic)
    Upload32(pic, 0, 0, width, height, picFormat, dataFormat, dataType, numMips,
             image, scaled);
...
void R_UpdateSubImage(image_t *image, byte *pic, int x, int y, int width,
  ...
  // TODO: This is fine for lightmaps but (unused) general RGBA images need to
  // store dataFormat / dataType in image_t for OpenGL ES?
  dataFormat = PixelDataFormatFromInternalFormat(image->internalFormat);
  dataType = picFormat == GL_RGBA16 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_BYTE;

  Upload32(pic, x, y, width, height, picFormat, dataFormat, dataType, 0, image,
           qfalse);
}
...
void R_LoadImage(const char *name, byte **pic, int *width, int *height,
  ...
  *numMips = 0;
  ...
  if (!*pic) {
    ...
    if (*pic)
      ri.Printf(PRINT_DEVELOPER,
                "WARNING: falling back to compressed texture %s when "
...
image_t *R_FindImageFile(const char *name, imgType_t type, imgFlags_t flags) {
  ...
  checkFlagsTrue =
      (imgFlags_t)(IMGFLAG_PICMIP | IMGFLAG_MIPMAP | IMGFLAG_GENNORMALMAP);
  ...
  if (r_normalMapping->integer && (picFormat == GL_RGBA8) &&
    ...
    normalFlags =
        (imgFlags_t)((flags & ~IMGFLAG_GENNORMALMAP) | IMGFLAG_NOLIGHTSCALE);
    ...
    if (normalImage == NULL) {
      ...
      RGBAtoNormal(pic, normalPic, width, height,
                   (qboolean)(flags & IMGFLAG_CLAMPTOEDGE));
      ...
      R_CreateImage(normalName, normalPic, normalWidth, normalHeight,
                    IMGTYPE_NORMAL, (imgFlags_t)(normalFlags), 0);
      ri.Free(normalPic);
    }
  }
  ...
  image = R_CreateImage2((char *)name, pic, width, height, picFormat,
                         picNumMips, type, (imgFlags_t)(flags), 0);
  ri.Free(pic);
  return image;
}
...
static void R_CreateDlightImage(void) {
  ...
  tr.dlightImage =
      R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE,
                    IMGTYPE_COLORALPHA, (imgFlags_t)(IMGFLAG_CLAMPTOEDGE), 0);
}
...
static void R_CreateFogImage(void) {
  ...
  tr.fogImage =
      R_CreateImage("*fog", (byte *)data, FOG_S, FOG_T, IMGTYPE_COLORALPHA,
                    (imgFlags_t)(IMGFLAG_CLAMPTOEDGE), 0);
...
static void R_CreateDefaultImage(void) {
  ...
  tr.defaultImage =
      R_CreateImage("*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE,
                    IMGTYPE_COLORALPHA, (imgFlags_t)(IMGFLAG_MIPMAP), 0);
}
...
void R_CreateBuiltinImages(void) {
  ...
  tr.whiteImage =
      R_CreateImage("*white", (byte *)data, 8, 8, IMGTYPE_COLORALPHA,
                    (imgFlags_t)(IMGFLAG_NONE), 0);
  if (r_dlightMode->integer >= 2) {
    for (x = 0; x < MAX_DLIGHTS; x++) {
      tr.shadowCubemaps[x] =
          R_CreateImage(va("*shadowcubemap%i", x), NULL, PSHADOW_MAP_SIZE,
                        PSHADOW_MAP_SIZE, IMGTYPE_COLORALPHA,
                        (imgFlags_t)(IMGFLAG_CLAMPTOEDGE | IMGFLAG_CUBEMAP), 0);
    }
  }
  ...
  tr.identityLightImage =
      R_CreateImage("*identityLight", (byte *)data, 8, 8, IMGTYPE_COLORALPHA,
                    (imgFlags_t)(IMGFLAG_NONE), 0);
  for (x = 0; x < 32; x++) {
    // scratchimage is usually used for cinematic drawing
    tr.scratchImage[x] =
        R_CreateImage("*scratch", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE,
                      IMGTYPE_COLORALPHA,
                      (imgFlags_t)(IMGFLAG_PICMIP | IMGFLAG_CLAMPTOEDGE), 0);
  }
  ...
  if (glRefConfig.framebufferObject) {
    ...
    tr.renderImage = R_CreateImage(
        ...
        (imgFlags_t)(IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE), hdrFormat);
    ...
    if (r_shadowBlur->integer || r_hdr->integer)
      tr.screenScratchImage = R_CreateImage(
          ...
          (imgFlags_t)(IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE),
          rgbFormat);

    if (r_shadowBlur->integer || r_ssao->integer)
      tr.hdrDepthImage = R_CreateImage(
          "*hdrDepth", NULL, width, height, IMGTYPE_COLORALPHA,
          (imgFlags_t)(IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE), GL_R32F);
    ...
    for (x = 0; x < 2; x++) {
      tr.textureScratchImage[x] = R_CreateImage(
          ...
          (imgFlags_t)(IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE), GL_RGBA8);
    }
    for (x = 0; x < 2; x++) {
    ...
    if (r_cubeMapping->integer) {
      tr.renderCubeImage = R_CreateImage(
          "*renderCube", NULL, r_cubemapSize->integer, r_cubemapSize->integer,
          IMGTYPE_COLORALPHA,
          (imgFlags_t)(IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE |
...
qhandle_t RE_RegisterSkin(const char *name) {
  ...
  skin->surfaces = (skinSurface_t *)ri.Hunk_Alloc(
