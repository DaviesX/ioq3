...
static qboolean RB_UpdateSunFlareVis(void) {
  ...
  // Note: On desktop OpenGL this is a sample count
  // (glRefConfig.occlusionQueryTarget == GL_SAMPLES_PASSED) but on OpenGL ES
  // this is a boolean (glRefConfig.occlusionQueryTarget ==
...
void RB_GaussianBlur(FBO_t *srcFbo, FBO_t *dstFbo, float blur) {
...

  {
    ...
    // first, downsample the framebuffer
    FBO_FastBlit(srcFbo, NULL, tr.quarterFbo[0], NULL, GL_COLOR_BUFFER_BIT,
                 GL_LINEAR);
    FBO_FastBlit(tr.quarterFbo[0], NULL, tr.textureScratchFbo[0], NULL,
                 GL_COLOR_BUFFER_BIT, GL_LINEAR);
