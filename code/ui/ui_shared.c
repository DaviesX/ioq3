      ...
      if (select != 0) {
        ...
        if (multiPtr->videoMode) {
          if (multiPtr->cvarValue[current] != -1) {
            ...
            Q_strncpyz(str, multiPtr->cvarStr[current],
                       MIN(x - multiPtr->cvarStr[current], sizeof(str)));
            w = atoi(str);
            h = atoi(x);
...
===============
*/
static void UI_ResolutionToAspect(const char *resolution, char *aspect,
                                  size_t aspectLength) {
  int i, w, h;
  char *x;
  char str[8];
...
static void Item_ApplyHacks(itemDef_t *item) {
  ...
  // Replace mode list and use a temporary ui_videomode cvar for handling custom
  // modes
  if (item->type == ITEM_TYPE_MULTI && item->cvar &&
      !Q_stricmp(item->cvar, "r_mode")) {
    multiDef_t *multiPtr = (multiDef_t *)item->typeData;
    int i, oldCount;
    ...
    if (*resbuf) {
      ...
      for (i = 0; builtinResolutions[i] && multiPtr->count < MAX_MULTI_CVARS;
           i++) {
        UI_ResolutionToAspect(builtinResolutions[i], aspect, sizeof(aspect));
        Com_sprintf(modeName, sizeof(modeName), "%s (%s)",
                    builtinResolutions[i], aspect);
    ...
    if (multiPtr->count < MAX_MULTI_CVARS) {
      ...
      Com_sprintf(currentResolution, sizeof(currentResolution), "%dx%d",
                  DC->glconfig.vidWidth, DC->glconfig.vidHeight);
      for (i = 0; i < multiPtr->count; i++) {
      ...
      if (i == multiPtr->count) {
        ...
        Com_sprintf(modeName, sizeof(modeName), "%s (%s)", currentResolution,
                    aspect);
    ...
    Com_Printf(
        "Found video mode list with %d modes, replaced list with %d modes\n",
        oldCount, multiPtr->count);
  }
}
