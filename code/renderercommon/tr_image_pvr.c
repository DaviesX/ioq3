...
typedef byte *(*pvr_image_func_t)(pvr_t *pvr, int offset, qboolean detwiddle,
...
static byte *decode_mm(pvr_t *pvr, pvr_image_func_t image_func,
                       pvr_pixel_func_t pixel_func, qboolean vq,
                       qboolean detwiddle) {
  int offset = vq ? mm_offset_vq(pvr->width) : mm_offset(pvr->width);
  return image_func(pvr, offset, detwiddle, pixel_func);
}

static byte *decode(pvr_t *pvr, int offset, qboolean detwiddle,
                    pvr_pixel_func_t pixel_func) {
  int x, y;
  unsigned int *rgba32;
  byte *ret;
...
static byte *decode_vq(pvr_t *pvr, int offset, qboolean detwiddle,
                       pvr_pixel_func_t pixel_func) {
  int x, y;
  unsigned int *rgba32;
  ...
  ret = (byte *)ri.Malloc(
      ...
      (pixel_func ? sizeof(unsigned int) : sizeof(unsigned short)));
  rgba32 = (unsigned int *)ret;
...
void R_LoadPVR(const char *name, byte **pic, int *width, int *height) {
  ...
  *pic = NULL;
  ...
  if (memcmp(ptr, "PVRT", 4) != 0)
    ri.Error(ERR_DROP, "LoadPVR: magic identifier does not match expected (%s)",
             name);
  ...
  default: {
    ri.Error(ERR_DROP, "LoadPVR: unsupported pixel type 0x%02x (%s)",
             pixel_type, name);
    break;
  }
  }
  ...
  default: {
    ri.Error(ERR_DROP, "LoadPVR: unsupported image type 0x%02x (%s)",
             image_type, name);
    break;
  }
  }
