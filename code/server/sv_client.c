...
void SV_DropClient(client_t *drop, const char *reason) {
  ...
  // This prevents SV_AddServerCommand() from making another recursive call to
  // SV_DropClient() if the client lacks sufficient space for another reliable
  // command it also guarantees that the client receives both the print and
  // disconnect commands
  drop->reliableSequence = drop->reliableAcknowledge;
  // Setting the gamestate message number to -1 ensures that
  // SV_AddServerCommand() will not call SV_DropClient() again, even though it
  // is unlikely the client will receive many server commands during the drop
...
int SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
  ...
  if (!cl->download) {
    ...
    if (!(sv_allowDownload->integer & DLF_ENABLE) ||
        ...
        (cl->downloadSize =
             FS_BaseDir_FOpenFileRead(cl->downloadName, &cl->download)) < 0) {
