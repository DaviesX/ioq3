...
qboolean CL_HTTP_Init() {
  ...
  const char *windowsVersion =
      GetVersionEx(&osvi)
          ? va("Windows %lu.%lu (build %lu)", osvi.dwMajorVersion,
               osvi.dwMinorVersion, osvi.dwBuildNumber)
          : "Windows";

  hInternet = InternetOpenA(va("%s %s", Q3_VERSION, windowsVersion),
...
void CL_HTTP_BeginDownload(const char *remoteURL) {
  ...
  hUrl = InternetOpenUrlA(
      ...
      (DWORD)-1,
      INTERNET_FLAG_HYPERLINK | INTERNET_FLAG_NO_CACHE_WRITE |
