...
void CL_NextDownload(void) {
  ...
  if (*clc.downloadName) {
    char *zippath = FS_BaseDir_BuildOSPath(
        Cvar_VariableString("fs_homedatapath"), clc.downloadName);
...
void CL_Frame(int msec) {
  ...
  if (clc.httpUsed) {
    ...
    if (finished) {
      ...
      FS_BaseDir_Rename_HomeData(clc.downloadTempName, clc.downloadName,
                                 qfalse);
      clc.downloadRestart = qtrue;
      CL_NextDownload();
    }
...
================
*/
static Q_PRINTF_FUNC(2, 3) void QDECL
...
void CL_InitRef(void) {
  ...
  Com_sprintf(dllName, sizeof(dllName), "renderer_%s" DLL_EXT,
              cl_renderer->string);
