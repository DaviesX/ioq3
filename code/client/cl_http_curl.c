...
static int CL_cURL_CallbackProgress(void *clientp, curl_off_t dltotal,
...
void CL_HTTP_BeginDownload(const char *remoteURL) {
  ...
  qcurl_easy_setopt_warn(downloadCURL, CURLOPT_REFERER,
                         va("ioQ3://%s", NET_AdrToString(clc.serverAddress)));
  qcurl_easy_setopt_warn(downloadCURL, CURLOPT_USERAGENT,
...
#else
  qcurl_easy_setopt_warn(downloadCURL, CURLOPT_PROTOCOLS,
  ...
  if (result != CURLM_OK) {
    ...
    downloadCURL = NULL;
    Com_Error(ERR_DROP,
              "CL_HTTP_BeginDownload: qcurl_multi_add_handle() failed: %s",
              qcurl_multi_strerror(result));
    return;
  }
}
