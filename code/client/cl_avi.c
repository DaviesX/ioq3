...
// Capture buffer stores RGB pixels but OpenGL ES reads RGBA and converts to RGB
// in-place. Encode buffer only needs to store RGB pixels. Allocate a bit more
// space for the capture buffer to account for possible padding at the end of
// pixel lines, and padding for alignment
#define MAX_PACK_LEN 16
  afd.cBuffer = Z_Malloc((afd.width * 4 + MAX_PACK_LEN - 1) * afd.height +
                         MAX_PACK_LEN - 1);
