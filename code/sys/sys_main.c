...
char *Sys_ParseProtocolUri(const char *uri) {
  ...
  if (!Q_strncmp(uri, "connect/", strlen("connect/"))) {
    ...
    for (i = 0; uri[i] != '\0'; i++) {
      if (uri[i] == '?') {
        // For forwards compatibility, any query string parameters are ignored
        // (e.g. "?password=abcd") However, these are not passed on macOS, so it
        // may be a bad idea to add them.
        break;
      }

      if (isalpha(uri[i]) == 0 && isdigit(uri[i]) == 0 && uri[i] != '.' &&
          uri[i] != ':' && uri[i] != '-' && uri[i] != '[' && uri[i] != ']') {
        Com_Printf(
            "Sys_ParseProtocolUri: hostname contains unsupported character.\n");
        return NULL;
      }
    }
...
#define MINSDL_VERSION                                                         \
  ...
  for (i = 1; i < argc; i++) {
    ...
    // For security reasons we always detect --uri, even when PROTOCOL_HANDLER
    // is undefined Any arguments after "--uri quake3://..." is ignored
