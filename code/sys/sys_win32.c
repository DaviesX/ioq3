...
#define MSSTORE_PATH "Quake 3"
  ...
  if (!microsoftStorePath[0]) {
    ...
    if (!SUCCEEDED(
            SHGetFolderPathA(NULL, CSIDL_PROGRAM_FILES, NULL, 0, szPath))) {
    ...
    Com_sprintf(microsoftStorePath, sizeof(microsoftStorePath),
                "%s%cModifiableWindowsApps%c%s%cEN", szPath, PATH_SEP, PATH_SEP,
                MSSTORE_PATH, PATH_SEP);
  }
...
qboolean Sys_OpenFolderInPlatformFileManager(const char *path) {
  return ShellExecute(NULL, "explore", path, NULL, NULL, SW_SHOWDEFAULT) >
         (HINSTANCE)32;
}
