...
char *Sys_HomeConfigPath(void) {
  ...
  if (!*homeConfigPath) {
    if ((p = getenv("XDG_CONFIG_HOME")) != NULL && *p != '\0')
      ...
      Com_sprintf(homeConfigPath, sizeof(homeConfigPath), "%s%c.config%c", p,
                  PATH_SEP, PATH_SEP);
...
char *Sys_HomeDataPath(void) {
  ...
  if (!*homeDataPath) {
    if ((p = getenv("XDG_DATA_HOME")) != NULL && *p != '\0')
      ...
      Com_sprintf(homeDataPath, sizeof(homeDataPath), "%s%c.local%cshare%c", p,
...
char *Sys_HomeStatePath(void) {
  ...
  if (!*homeStatePath) {
    if ((p = getenv("XDG_STATE_HOME")) != NULL && *p != '\0')
      ...
      Com_sprintf(homeStatePath, sizeof(homeStatePath), "%s%c.local%cstate%c",
...
static qboolean Sys_ShouldUseLegacyHomePath(void) {
  ...
  if (access(migrationRefusedPath, F_OK) != 0) {
    dialogResult_t result = Sys_Dialog(
        DT_YES_NO,
        va("Modern games and applications store files in "
           ...
           PRODUCT_NAME, Sys_HomeConfigPath(), Sys_HomeDataPath(),
           Sys_HomeStatePath(), legacyHomePath),
...
#ifdef OPEN_MAX
    // On older macOS versions an error can happen trying to set a file limit
    // above OPEN_MAX. If we see an error, then try again with OPEN_MAX as the
    // limit.
