...
qboolean FS_FileExists_HomeData(const char *file) {
  return FS_FileInPathExists(
      FS_BuildOSPath(fs_homedatapath->string, fs_gamedir, file));
}
...
static qboolean FS_BaseDir_FileExists_HomeData(const char *file) {
  return FS_FileInPathExists(
      FS_BaseDir_BuildOSPath(fs_homedatapath->string, file));
}
...
===========
*/
static fileHandle_t FS_OSPath_FOpenFileWrite(const char *ospath,
                                             const char *filename) {
  fileHandle_t f;
...
long FS_BaseDir_FOpenFileRead(const char *filename, fileHandle_t *fp) {
  ...
  for (int i = 0; i < ARRAY_LEN(fs_pathVars) && !fsh[f].handleFiles.file.o;
       i++) {
...
===========
*/
void FS_BaseDir_Rename_HomeData(const char *from, const char *to,
                                qboolean safe) {
...
fileHandle_t FS_FOpenFileWrite_HomeConfig(const char *filename) {
  return FS_OSPath_FOpenFileWrite(
      FS_BuildOSPath(fs_homeconfigpath->string, fs_gamedir, filename),
      filename);
}
...
long FS_FOpenFileReadDir(const char *filename, searchpath_t *search,
  ...
  *file = FS_HandleForFile();
  ...
  if (search->pack) {
    ...
    // used FS_FileExists_HomeData instead
...
int FS_GetModList(char *listbuf, int bufsize) {
  int nMods, i, j, k, nTotal, nLen, nPaks = 0, nDirs = 0, nPakDirs = 0,
                                    nPotential, nDescLen;
  char **pFiles = NULL;
  char **pPaks = NULL;
  char **pDirs = NULL;
  ...
  for (i = 0; i < fs_numServerReferencedPaks; i++) {
    ...
    if (!havepak && fs_serverReferencedPakNames[i] &&
      ...
      if (dlstring) {
        ...
        if (FS_BaseDir_FileExists_HomeData(
...
static void FS_Startup(const char *gameName) {
  ...
  fs_homeconfigpath =
      Cvar_Get("fs_homeconfigpath", configPath, CVAR_INIT | CVAR_PROTECTED);
  fs_homedatapath =
      Cvar_Get("fs_homedatapath", dataPath, CVAR_INIT | CVAR_PROTECTED);
  fs_homestatepath =
      Cvar_Get("fs_homestatepath", statePath, CVAR_INIT | CVAR_PROTECTED);
  ...
  fs_microsoftstorepath =
      Cvar_Get("fs_microsoftstorepath", Sys_MicrosoftStorePath(),
               CVAR_INIT | CVAR_PROTECTED);
...
===================
*/
static void FS_AppendUserFriendlyPakList(char *buf, size_t bufsize,
                                         int foundPakFiles, int numPakFiles) {
  int missingCount = 0;
  int missingIndex = 0;
  ...
  if (missingConsecutive && missingMax >= missingMin + 2) {
    Q_strcat(
        buf, bufsize,
        va(" \"pak%d.pk3\" through \"pak%d.pk3\"", missingMin, missingMax));
  } else {
    for (i = 0; i < numPakFiles; i++) {
      if (foundPakFiles & (1 << i)) {
        continue;
...
static void FS_CheckPak0(void) {
  ...
  if (!com_standalone->integer &&
      (foundPak & ((1 << NUM_ID_PAKS) - 1)) != ((1 << NUM_ID_PAKS) - 1)) {
    char errorText[MAX_STRING_CHARS] = "";
    char gamePath[MAX_OSPATH];

    Com_sprintf(gamePath, sizeof(gamePath), "%s%c%s%c", installPath, PATH_SEP,
                BASEGAME, PATH_SEP);

    Q_strcat(errorText, sizeof(errorText),
             "Quake 3 data files are missing. Please copy");

    FS_AppendUserFriendlyPakList(errorText, sizeof(errorText), foundPak,
                                 NUM_ID_PAKS);

    Q_strcat(errorText, sizeof(errorText),
             va(" from the \"%s\" directory in your Quake 3 install or CD-ROM "
                "to:\n\n"
                "%s\n\n",
                BASEGAME, gamePath));
  ...
  if (!com_standalone->integer &&
      (foundTA & ((1 << NUM_TA_PAKS) - 1)) != ((1 << NUM_TA_PAKS) - 1) &&
      (!Q_stricmp(fs_gamedirvar->string, BASETA) ||
       !Q_stricmp(fs_basegame->string, BASETA))) {
    char errorText[MAX_STRING_CHARS] = "";
    char gamePath[MAX_OSPATH];
    ...
    FS_AppendUserFriendlyPakList(errorText, sizeof(errorText), foundTA,
                                 NUM_TA_PAKS);

    Q_strcat(errorText, sizeof(errorText),
             va(" from the \"%s\" directory in your Quake 3 Team Arena install "
                "or CD-ROM to:\n\n"
                "%s\n\n",
                BASETA, gamePath));
...
void FS_FilenameCompletion(const char *dir, const char *ext, char *filter,
  ...
  filenames = FS_ListFilteredFiles(dir, ext, filter, &nfiles, qtrue,
